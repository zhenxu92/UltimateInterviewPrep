...
public class AddBoldTag {
    ...
    public static String addBoldTag(String s, String[] dict) {
        ...
        for (String str : dict) {
            ...
            index = s.indexOf(str);
            while (index != -1) {
                pq.add(new int[]{index, index + str.length()});
                index += 1;
        ...
        while (!pq.isEmpty()) {
            int[] temp = pq.remove();
            sb.append(s.substring(start, temp[0]));
            sb.append("<b>");
            sb.append(s.substring(temp[0], temp[1]));
            sb.append("</b>");
            start = temp[1];
        }
    ...
    private static Stack<int[]> merge(PriorityQueue<int[]> pq) {
        Stack<int[]> stack = new Stack<int[]>();
        if (pq.isEmpty()) return stack;
        int[] temp = pq.remove();
        stack.push(temp);
        while (!stack.isEmpty() && !pq.isEmpty()) {
            ...
            if (other[0] > cur[1]) {
                ...
                int[] merge = new int[]{cur[0], Math.max(cur[1], other[1])};
                stack.push(merge);
            }
        }
        return stack;
